...
import './PrismaticBurst.css'; // Make sure to use the new CSS below
...
void main(){
    ...
    // Invert colors for light theme (so it darkens the background)
    // col = 1.0 - col; // ✨ REMOVED THIS LINE FOR DARK MODE
...
// Default color palette (these will now be bright, emissive colors)
const defaultColors = ['#E63946', '#D62828', '#A41623', '#6A0572', '#F77F00'];
...
const PrismaticBurst = ({
  ...
  hoverDampness = 0.1,
  rayCount,
  mixBlendMode = 'screen' // ✨ Changed to 'screen' for dark backgrounds
}) => {
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      dpr,
      alpha: true, // Set to true to allow background to show through
      antialias: false
    });
